...
warnings.filterwarnings("ignore", message="Unverified HTTPS request")  # Подавляем предупреждения
...
def extract_bus_data(url):
    try:
        ...
        if response.status_code == 200:
            ...
            # Дальше ваш код для обработки данных...
...
# Список URL-адресов для автобусов, микроавтобусов, минивэнов и легковых автомобилей
urls = [
    ...
    "https://bbus.ru/catalog/hongqi-hq9",
    "https://bbus.ru/catalog/gac-gn8-6-mest"]
extract_bus_data(urls)

# Создаём папку datasets, если она ещё не существует
datasets_folder = "datasets"
if not os.path.exists(datasets_folder):
    os.makedirs(datasets_folder)

# Функция для корректировки названия
def clean_bus_name(name):
    words_to_remove = ["автобуса", "городского автобуса", "городского", "микро","минивэна ", "автомобиля "]  # Список лишних слов
...
def extract_bus_data(url):
    ...
    if response.status_code == 200:
        ...
        # Извлечение заголовка страницы (название)
        ...
        ul = soup.find('ul', class_='ul-horizontal-auto')  # Список с характеристиками
        if ul:
        ...
        if rent_section:
            rows = rent_section.find_all('tr')[2:]  # Пропускаем заголовки
            for row in rows:
        ...
        if transfer_section:
            ...
            if transfer_table_html:
                rows = transfer_table_html.find_all("tr")[2:]  # Пропускаем заголовки
                for row in rows:
                    cols = row.find_all("td")
        ...
        print(f"Ошибка при подключении к сайту {url}: {response.status_code}")
        return None, None, None

# Сбор данных по всем URL
all_details = []
all_rent_prices = []
all_transfer_prices = []
...
df_details.to_csv(os.path.join(datasets_folder, "bus_details.csv"), index=False, encoding="utf-8")
df_rent.to_csv(os.path.join(datasets_folder, "rent_prices.csv"), index=False, encoding="utf-8")
df_transfer.to_csv(os.path.join(datasets_folder, "transfer_prices.csv"), index=False, encoding="utf-8")

print("Данные успешно сохранены в:")
print(f"- {datasets_folder}/bus_details.csv")
print(f"- {datasets_folder}/rent_prices.csv")
print(f"- {datasets_folder}/transfer_prices.csv")
